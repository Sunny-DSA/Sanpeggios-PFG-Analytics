...
const StoreUI = {
  ...
  setupFileUpload: function () {
    ...
    uploadUI.innerHTML =
      ...
      "<h3>Click to Browse Invoice Files</h3>" +
      "<p>Files must be in COMMA format from PFG</p>" +
    ...
    multiFileInput.addEventListener(
      "change",
      function (e) {
  ...
  handleFiles: async function (files) {
    ...
    if (successCount > 0) {
      ...
      const loadResult = await StoreDataManager.loadFromDatabase();
      console.log(
        `Reloaded ${loadResult.recordCount} total records from database`,
      );
  ...
  showUploadResults: function (results, successCount, errorCount) {
    ...
    let html =
      ...
      "<p>✅ " +
      successCount +
      " files processed successfully</p>";
    ...
    results.forEach(function (result) {
      if (result.success) {
        ...
        if (result.fileInfo.unassignedRecords > 0) {
          html +=
            '<span class="warning-badge">⚠️ ' +
            result.fileInfo.unassignedRecords +
            " unassigned records</span>";

          // Show diagnostic information for unassigned records
          if (
            result.fileInfo.unassignedSamples &&
            result.fileInfo.unassignedSamples.length > 0
          ) {
            html +=
              '<div class="unassigned-diagnostic">' +
  ...
  deleteFile: async function (storeId, fileId) {
    ...
    if (success) {
      ...
      alert(
        "File removed. Please re-upload all files for this store to refresh the data.",
      );
      this.updateFileList(storeId);
    }
  },
  ...
  createFileListSection: function () {
    ...
    section.innerHTML =
      "<h2>📄 Store Files</h2>" + '<div id="storeFileList"></div>';
  ...
  updateStoreKPIs: function (storeId) {
    ...
    if (!storeData || !storeData.kpis) {
      container.innerHTML =
        '<p class="text-muted">No KPI data available for this store</p>';
      return;
    }
    ...
    let html =
      ...
      '<span class="' +
      (kpis.revenueGrowth > 0 ? "positive" : "negative") +
      '">' +
      ...
      "</span> MoM Growth" +
      "</div>" +
      '<div class="kpi-rank">Rank #' +
      kpis.revenueRank +
      " of " +
      Object.keys(STORE_CONFIG).length +
      "</div>" +
      "</div>" +
      // Efficiency KPIs
      '<div class="kpi-card">' +
      ...
      '<div class="kpi-metric">' +
      kpis.orderFrequency.toFixed(1) +
      " orders/week</div>" +
      '<div class="kpi-rank">Efficiency Rank #' +
      kpis.efficiencyRank +
      "</div>" +
      "</div>" +
      // Product KPIs
      ...
      "<h4>Risk Metrics</h4>" +
      '<div class="kpi-value">' +
      kpis.priceStability.toFixed(1) +
      "%</div>" +
      '<div class="kpi-label">Price Stability</div>' +
      '<div class="kpi-metric">Vendor HHI: ' +
      kpis.vendorConcentration.toFixed(0) +
      "</div>" +
      '<div class="kpi-metric ' +
      (kpis.vendorConcentration > 2500 ? "negative" : "positive") +
      '">' +
      (kpis.vendorConcentration > 2500 ? "High" : "Low") +
      " concentration risk" +
      "</div>" +
      "</div>" +
      "</div>";
